...
const fetchProductPage = async () => {
  ...
  productCardContainer[0].innerHTML += productProductPage.map((item) => {
    let lenses = "";
    if (item.lenses.length >= 2) {
      lenses += `<select class="lense">
      <option value=""> Personnaliez votre lense</option> `;
      for (i = 0; i < item.lenses.length; i++) {
        if (item.lenses[i]) {
          lenses += `<option value=" ${item.lenses[i]}"> ${item.lenses[i]} </option>`;
        }
      }
      lenses += `</select>`;
    } else if (item.lenses.length == 1) {
      lenses = `<p> ${item.lenses[0]} </p>
      `;
    }
    return `
      <div class="row">
